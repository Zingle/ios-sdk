...
/**
 *  `ZNGAvatarImageFactory` is a factory that provides a means for creating and styling
 *  `ZNGAvatarImage` objects to be displayed in a `ZNGCollectionViewCell` of a `ZNGCollectionView`.
 */
@interface ZNGAvatarImageFactory : NSObject

/**
*  Creates and returns a `ZNGAvatarImage` object with the specified placeholderImage that is
...
*
*  @return An initialized `ZNGAvatarImage` object if created successfully, `nil` otherwise.
*/
+ (ZNGAvatarImage *)avatarImageWithPlaceholder:(UIImage *)placeholderImage diameter:(NSUInteger)diameter;

/**
 *  Creates and returns a `ZNGAvatarImage` object with the specified image that is
 *  cropped to a circle of the given diameter and used for the `avatarImage` and `avatarPlaceholderImage` properties
 *  of the returned `ZNGAvatarImage` object. This image is then copied and has a transparent black mask applied to it, 
 *  which is used for the `avatarHighlightedImage` property of the returned `ZNGAvatarImage` object.
 *
...
/**
 *  Creates and returns a `ZNGAvatarImage` object with a circular shape that displays the specified userInitials
