 ...
 *  a `ZNGBaseViewController` and `ZNGCollectionView` interact with media message model objects.
 *
...
/**
 *  @return The frame size for the mediaView when displayed in a `ZNGCollectionViewCell`. 
 *
 *  @discussion You should return an appropriate size value to be set for the mediaView's frame
 *  based on the contents of the view, and the frame and layout of the `ZNGCollectionViewCell`
 ...
 *  same value here as mediaView. Otherwise, consider using `ZNGMediaPlaceholderView`.
 *
 *  @warning You must not return `nil` from this method.
 *
 *  @see ZNGMediaPlaceholderView.
 */
