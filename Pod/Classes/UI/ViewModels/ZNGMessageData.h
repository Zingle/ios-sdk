...
/**
 *  The `ZNGMessageData` protocol defines the common interface through which 
 *  a `ZNGBaseViewController` and `ZNGCollectionView` interact with message model objects.
 *
 *  It declares the required and optional methods that a class must implement so that instances of that class 
 *  can be displayed properly within a `ZNGCollectionViewCell`.
 *
 *  The class that conforms to this protocol is provided in the library. See `ZNGMessage`.
 *
 *  @see ZNGMessage.
 */
@protocol ZNGMessageData <NSObject>
 ...
 *  If this method returns `YES`, an instance of `ZNGBaseViewController` will ignore 
 *  the `text` method of this protocol when dequeuing a `ZNGCollectionViewCell`
