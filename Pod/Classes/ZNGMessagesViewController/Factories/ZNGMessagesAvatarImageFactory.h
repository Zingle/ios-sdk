//
//  ZNGMessagesAvatarImageFactory.h
//  Pods
//
...
/**
 *  `ZNGMessagesAvatarImageFactory` is a factory that provides a means for creating and styling
 *  `ZNGMessagesAvatarImage` objects to be displayed in a `ZNGMessagesCollectionViewCell` of a `ZNGMessagesCollectionView`.
 */
@interface ZNGMessagesAvatarImageFactory : NSObject

/**
*  Creates and returns a `ZNGMessagesAvatarImage` object with the specified placeholderImage that is
...
*
*  @return An initialized `ZNGMessagesAvatarImage` object if created successfully, `nil` otherwise.
*/
+ (ZNGMessagesAvatarImage *)avatarImageWithPlaceholder:(UIImage *)placeholderImage diameter:(NSUInteger)diameter;

/**
 *  Creates and returns a `ZNGMessagesAvatarImage` object with the specified image that is
 *  cropped to a circle of the given diameter and used for the `avatarImage` and `avatarPlaceholderImage` properties
 *  of the returned `ZNGMessagesAvatarImage` object. This image is then copied and has a transparent black mask applied to it, 
 *  which is used for the `avatarHighlightedImage` property of the returned `ZNGMessagesAvatarImage` object.
 *
...
/**
 *  Creates and returns a `ZNGMessagesAvatarImage` object with a circular shape that displays the specified userInitials
